...
const Detail = () => {
  ...
  // Fetch post details
  const { data: postDetail, isLoading: isFetchingPostDetail } = useQuery({
    queryKey: ["detailPost"],
    queryFn: async () => {
  ...
  const myPost = postDetail?.user._id === logedInUser?._id;
  ...
  return (
    <>
      {isFetchingPostDetail && <DetailPageSkeleton />}
                        ...
                        <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center p-4">
                          <div className="bg-white p-4 rounded-lg shadow-lg max-w-xs w-full md:max-w-md">
                            ...
                            <Link to={`/update/${postDetail?._id}`}>
                              <button className="btn btn-accent w-full mb-2 text-sm md:text-base">
                                Update
                              </button>
                            </Link>
                            <button
                              onClick={() => deletePost(postDetail?._id)}
                              className="btn btn-orange w-full text-sm md:text-base"
                            >
                              {isDeleting ? (
